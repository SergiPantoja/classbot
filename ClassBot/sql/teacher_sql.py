...
def get_teachers_by_classroom(classroom_id: int) -> list[Teacher]:
    """ Returns a list of teachers for the given classroom. """
    with session() as s:
        return s.query(Teacher).join(Teacher_classroom).filter(Teacher_classroom.classroom_id == classroom_id).all()
